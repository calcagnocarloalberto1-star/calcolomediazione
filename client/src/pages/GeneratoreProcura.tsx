...
${delegato.nomeCognome ? `l'Avv. ${delegato.nomeCognome}` : "l'Avv. ____________________"}${delegato.luogoNascita ? `, nato/a a ${delegato.luogoNascita}` : ""}${delegato.dataNascita ? ` il ${delegato.dataNascita}` : ""}${indirizzoDelegato ? `, con studio in ${indirizzoDelegato}` : ""}${delegato.codiceFiscale ? `, C.F. ${delegato.codiceFiscale.toUpperCase()}` : ""}${delegato.partitaIva ? `, P.IVA ${delegato.partitaIva}` : ""}${delegato.telefono ? `, Tel. ${delegato.telefono}` : ""}${delegato.cellulare ? `, Cell. ${delegato.cellulare}` : ""}${delegato.email ? `, Email: ${delegato.email}` : ""}${delegato.pec ? `, PEC: ${delegato.pec}` : ""},
...
async function generaDocx(
  ...
  // Build delegato description
  let delegatoText = `${delegato.nomeCognome ? `l'Avv. ${delegato.nomeCognome}` : "l'Avv. ____________________"}`;
  ...
  if (indirizzoDelegato) delegatoText += `, con studio in ${indirizzoDelegato}`;
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "PROCURA SPECIALE PER LA PARTECIPAZIONE ALLA MEDIAZIONE",
                bold: true,
                size: 28,
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "ai sensi dell'art. 8, commi 4 e 4-bis, D.Lgs. n. 28/2010",
                italics: true,
                size: 22,
                font: "Times New Roman",
              }),
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "La presente procura è conferita ai sensi e per gli effetti dell'art. 8, commi 4 e 4-bis, del D.Lgs. n. 28/2010, come modificato dal D.Lgs. n. 149/2022 (Riforma Cartabia).",
                size: 24,
                font: "Times New Roman",
              }),
            ],
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "Si dichiara che la presente procura è conferita a titolo speciale e che il/la delegato/a è a conoscenza dei fatti oggetto della controversia.",
                size: 24,
                font: "Times New Roman",
              }),
            ],
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: `Luogo e data: ______________________, ${oggi}`,
                size: 24,
                font: "Times New Roman",
              }),
            ],
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: "Firma del delegante: ____________________________",
                size: 24,
                font: "Times New Roman",
              }),
            ],
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: delegante.nomeCognome || "(Nome e Cognome del Delegante)",
                size: 24,
                font: "Times New Roman",
                italics: !delegante.nomeCognome,
              }),
            ],
          }),
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: delegato.nomeCognome || "(Nome e Cognome dell'Avvocato)",
                size: 24,
                font: "Times New Roman",
                italics: !delegato.nomeCognome,
              }),
            ],
          }),
        ],
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Art. 8, commi 4 e 4-bis, D.Lgs. n. 28/2010
              </p>
            </div>
          </div>
          <p className="text-muted-foreground max-w-2xl">
            Genera automaticamente la procura speciale per la partecipazione alla
            mediazione. Compila i dati e scarica il documento in formato Word
            (.docx) o PDF, conforme al modello previsto dalla normativa vigente.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField
                ...
                value={delegante.nomeCognome}
                onChange={(v) => updateDelegante("nomeCognome", v)}
                required
                className="sm:col-span-2"
                testId="input-delegante-nome"
              />
              <FormField
                label="Luogo di Nascita"
                value={delegante.luogoNascita}
                onChange={(v) => updateDelegante("luogoNascita", v)}
                testId="input-delegante-luogo-nascita"
              />
              <FormField
                label="Data di Nascita"
                value={delegante.dataNascita}
                onChange={(v) => updateDelegante("dataNascita", v)}
                type="date"
                testId="input-delegante-data-nascita"
              />
              <FormField
                label="Residenza (Via/Piazza)"
                value={delegante.indirizzo}
                onChange={(v) => updateDelegante("indirizzo", v)}
                className="sm:col-span-2"
                testId="input-delegante-indirizzo"
              />
              <FormField
                label="Comune"
                value={delegante.comune}
                onChange={(v) => updateDelegante("comune", v)}
                testId="input-delegante-comune"
              />
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    value={delegante.provincia}
                    onValueChange={(v) => updateDelegante("provincia", v)}
                  >
                ...
                <FormField
                  label="CAP"
                  value={delegante.cap}
                  onChange={(v) => updateDelegante("cap", v)}
                  testId="input-delegante-cap"
                />
              </div>
              <FormField
                label="Codice Fiscale"
                value={delegante.codiceFiscale}
                onChange={(v) => updateDelegante("codiceFiscale", v.toUpperCase())}
                testId="input-delegante-cf"
              />
              <FormField
                label="P.IVA (opzionale)"
                value={delegante.partitaIva}
                onChange={(v) => updateDelegante("partitaIva", v)}
                testId="input-delegante-piva"
              />
              ...
              <FormField
                ...
                value={delegante.numeroDocumento}
                onChange={(v) => updateDelegante("numeroDocumento", v)}
                testId="input-delegante-num-doc"
              />
              <FormField
                label="Ente Rilascio"
                value={delegante.enteRilascio}
                onChange={(v) => updateDelegante("enteRilascio", v)}
                placeholder="es. Comune di Genova"
                testId="input-delegante-ente"
              />
              <FormField
                label="Data Rilascio"
                value={delegante.dataRilascio}
                onChange={(v) => updateDelegante("dataRilascio", v)}
                type="date"
                testId="input-delegante-data-rilascio"
              />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-2 border-foreground shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField
                label="Nome e Cognome"
                value={delegato.nomeCognome}
                onChange={(v) => updateDelegato("nomeCognome", v)}
                required
                className="sm:col-span-2"
                testId="input-delegato-nome"
              />
              <FormField
                label="Luogo di Nascita"
                value={delegato.luogoNascita}
                onChange={(v) => updateDelegato("luogoNascita", v)}
                testId="input-delegato-luogo-nascita"
              />
              <FormField
                label="Data di Nascita"
                value={delegato.dataNascita}
                onChange={(v) => updateDelegato("dataNascita", v)}
                type="date"
                testId="input-delegato-data-nascita"
              />
              <FormField
                label="Studio/Residenza (Via/Piazza)"
                value={delegato.indirizzo}
                onChange={(v) => updateDelegato("indirizzo", v)}
                className="sm:col-span-2"
                testId="input-delegato-indirizzo"
              />
              <FormField
                label="Comune"
                value={delegato.comune}
                onChange={(v) => updateDelegato("comune", v)}
                testId="input-delegato-comune"
              />
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1.5 block">
                  ...
                  <Select
                    value={delegato.provincia}
                    onValueChange={(v) => updateDelegato("provincia", v)}
                  >
                ...
                <FormField
                  label="CAP"
                  value={delegato.cap}
                  onChange={(v) => updateDelegato("cap", v)}
                  testId="input-delegato-cap"
                />
              </div>
              <FormField
                label="Codice Fiscale"
                value={delegato.codiceFiscale}
                onChange={(v) => updateDelegato("codiceFiscale", v.toUpperCase())}
                testId="input-delegato-cf"
              />
              <FormField
                label="P.IVA"
                value={delegato.partitaIva}
                onChange={(v) => updateDelegato("partitaIva", v)}
                testId="input-delegato-piva"
              />
              ...
              <FormField
                label="Telefono"
                value={delegato.telefono}
                onChange={(v) => updateDelegato("telefono", v)}
                type="tel"
                testId="input-delegato-telefono"
              />
              <FormField
                label="Cellulare"
                value={delegato.cellulare}
                onChange={(v) => updateDelegato("cellulare", v)}
                type="tel"
                testId="input-delegato-cellulare"
              />
              <FormField
                label="Email"
                value={delegato.email}
                onChange={(v) => updateDelegato("email", v)}
                type="email"
                testId="input-delegato-email"
              />
              <FormField
                label="PEC"
                value={delegato.pec}
                onChange={(v) => updateDelegato("pec", v)}
                type="email"
                testId="input-delegato-pec"
              />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField
                ...
                value={procedimento.numeroProcedimento}
                onChange={(v) => updateProcedimento("numeroProcedimento", v)}
                required
                testId="input-num-procedimento"
              />
              <FormField
                label="Organismo di Mediazione"
                value={procedimento.organismoDiMediazione}
                onChange={(v) => updateProcedimento("organismoDiMediazione", v)}
                placeholder="es. Camera di Mediazione di Genova"
                testId="input-organismo"
              />
              <div className="sm:col-span-2">
                ...
                <Select
                  value={procedimento.materia}
                  onValueChange={(v) => updateProcedimento("materia", v)}
                >
              ...
              <CardContent className="p-8">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Button
                    ...
                    data-testid="button-chiudi-anteprima"
                  >
                    Chiudi
                  </Button>
                </div>
                ...
                <div
                  className="prose prose-sm max-w-none font-serif whitespace-pre-wrap leading-relaxed text-foreground/90"
        ...
        <div className="text-center text-xs text-muted-foreground pb-8 space-y-1">
          <p>
            Documento generato automaticamente da CalcoloMediazione.
          </p>
          <p>
            La procura è conforme al modello previsto dall'art. 8, commi 4 e 4-bis,
            D.Lgs. n. 28/2010, come modificato dal D.Lgs. n. 149/2022.
          </p>
          <p>
            Il presente strumento non sostituisce la consulenza legale professionale.
          </p>
        </div>
      </div>
    </div>
