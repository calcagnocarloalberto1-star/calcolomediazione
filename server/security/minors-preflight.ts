...
// sovraordinata e indipendente: se disattivato, nessuna di queste rotte è
...
// non sono condivisi tra più istanze del server. Una persistenza durevole
// (tabella dedicata con migrazione Drizzle) è un'attività di follow-up che
...
// come "difesa in profondità": una seconda verifica troverebbe il token già
...
if (!PREFLIGHT_SECRET_CONFIGURED) {
  console.warn(
    ...
    "comunque solo pochi minuti, quindi l'impatto pratico è limitato). Imposta MINORS_PREFLIGHT_SECRET come " +
...
// Registro dei token emessi e non ancora consumati (monouso). La chiave è il
// jti. Il consumo è "get poi delete" senza alcun `await` frapposto: essendo
 ...
 * classificazione non può provenire dallo stesso contenuto che il gate deve
 * poter bloccare.
 */
  ...
  app.post("/api/analisi/minori-preflight", (req: Request, res: Response) => {
    if (!isCaseAiEnabled()) {
      return res.status(503).json({
        error: "L'Analisi AI è temporaneamente sospesa per completare le verifiche privacy e contrattuali.",
    ...
    if (!noticeVersion || noticeVersion !== MINORS_NOTICE_VERSION) {
      return res.status(422).json({
        error: "L'avviso privacy mostrato non è aggiornato: ricarica la pagina e riprova.",
