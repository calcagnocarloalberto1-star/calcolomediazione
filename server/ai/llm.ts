...
1) Rispondere alla richiesta in linguaggio naturale, in italiano professionale e chiaro, in un campo "risposta" (poche frasi): spiega che cosa hai fatto, quante parti hai individuato (istanti e aderenti) e con quali ruoli, QUALI CAMPI restano vuoti perche' l'informazione non risulta nemmeno indirettamente dai documenti forniti (elencali per parte, non genericamente), ed eventuali osservazioni di rischio o anomalie emerse dai documenti. Per ogni campo rimasto vuoto, quando ti sembra utile, aggiungi un suggerimento per il mediatore su che cosa indicare se la parte e il suo avvocato non fossero in grado di fornire quell'elemento (es. "assente" o "non fornito dalla parte" invece di un campo lasciato ambiguo) — resta un suggerimento per il mediatore, mai un valore che inserisci tu stesso nel modulo.
...
Compila il modulo nel modo piu' completo possibile: per OGNI campo, prima di lasciarlo vuoto, verifica se puo' essere ragionevolmente ricavato incrociando le informazioni presenti nell'insieme dei documenti forniti, anche quando nessun documento lo dichiara in modo letterale (es. la cittadinanza puo' risultare dal documento di identita' anche se l'istanza non la menziona espressamente; la modalita' di identificazione puo' risultare dal tipo di documento effettivamente allegato). NON inventare MAI un valore privo di fondamento nei documenti: lascia il campo vuoto SOLO quando, incrociando tutti i documenti, l'elemento non e' davvero ricostruibile nemmeno indirettamente, e segnalalo in "risposta" come indicato al punto 1 — mai un default "rassicurante" o un valore scelto a caso solo per non lasciare il campo vuoto. Per i campi di "valutazione del rischio" (quelli descritti come "Suggerimento di rischio" o "Proposta") puoi formulare una valutazione professionale ragionevole in base al contenuto dei documenti, ma resta un SUGGERIMENTO che l'utente dovra' confermare, e va formulata per CIASCUNA parte separatamente (il rischio di un istante puo' essere diverso da quello dell'aderente).
